...
ipcRenderer.once("getThemeOverride", (e, theme) => {
    if (theme !== null) {
...
ipcRenderer.once("getKbOverride", (e, layout) => {
    if (layout !== null) {
...
function initGraphicalErrorHandling() {
    ...
    window.onerror = (msg, path, line, col, error) => {
        ...
        ipcRenderer.send("log", "error", `${error}: ${msg}`);
        ipcRenderer.send("log", "debug", `at ${path} ${line}:${col}`);
    };
}
...
window.themeChanger = theme => {
    ipcRenderer.send("setThemeOverride", theme);
    setTimeout(() => {
...
window.remakeKeyboard = layout => {
    ...
    ipcRenderer.send("setKbOverride", layout);
};
...
window.focusShellTab = number => {
    ...
    if (number !== window.currentTerm && window.term[number]) {
        ...
        ipcRenderer.once("ttyspawn-reply", (e, r) => {
...
window.openSettings = async () => {
    ...
    new Modal({
        ...
        html: `<table id="settingsEditor">
                ...
                <br>`,
        buttons: [
            {label: "Open in External Editor", action:`shell.openPath('${settingsFile}');electronWin.minimize();`},
...
window.toggleFullScreen = () => {
    let useFullscreen = (remote.getCurrentWindow().isFullScreen() ? false : true);
    remote.getCurrentWindow().setFullScreen(useFullscreen);
...
window.openShortcutsHelp = () => {
    ...
    new Modal({
        ...
        html: `<h5>Using either the on-screen or a physical keyboard, you can use the following shortcuts:</h5>
                ...
                <br>`,
        buttons: [
            {label: "Open Shortcuts File", action:`shell.openPath('${shortcutsFile}');electronWin.minimize();`},
            {label: "Reload UI", action: "window.location.reload(true);"},
        ]
    }, () => {
        window.keyboard.attach();
